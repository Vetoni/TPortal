<?php
/**
 * Message translations.
 *
 * This file is automatically generated by 'yii message' command.
 * It contains the localizable messages extracted from source code.
 * You may modify this file by translating the extracted messages.
 *
 * Each array element represents the translation (value) of a message (key).
 * If the value is empty, the message is considered as not translated.
 * Messages that no longer need translation will have their translations
 * enclosed between a pair of '@@' marks.
 *
 * Message string can be used with plural forms format. Check i18n section
 * of the guide for details.
 *
 * NOTE: this file must be saved in UTF-8 encoding.
 */
return [
    'Contact page' => 'Контакты',
    'FAQ page' => 'Вопросы',
    'Feedback page' => 'Обратная связь',
    'Message' => 'Сообщение',
    'Send' => 'Отправить',
    'Services page' => 'Услуги',
    'Thank you for the feedback!' => 'Спасибо за ваш отзыв!',
    'About company' => 'О компании',
    'Address' => 'Адрес',
    'All regions' => 'Все страны',
    'Announce' => 'Анонс',
    'Cancel' => 'Отмена',
    'Changed date' => 'Дата изменения',
    'Choose a city' => 'Выбрать город',
    'Choose a region' => 'Выбрать страну',
    'Choose a sub type' => 'Выберите подвид туров',
    'Choose a type' => 'Выберите вид туров',
    'Cities' => 'Города',
    'City' => 'Город',
    'City list' => 'Список городов',
    'Content' => 'Содержимое',
    'Content pages' => 'Страницы содержимого',
    'Create' => 'Создать',
    'Create City' => 'Создать город',
    'Create Region' => 'Создать страну',
    'Create Tour' => 'Создать тур',
    'Create Tour type' => 'Создать вид туров',
    'Create city' => 'Создать город',
    'Create content page' => 'Создать страницу содержимого',
    'Create news item' => 'Создать новость',
    'Create region' => 'Создать страну',
    'Create tour' => 'Создать тур',
    'Create tour type' => 'Создать вид туров',
    'Created date' => 'Дата создания',
    'Dashboard' => 'Администрирование',
    'Description' => 'Описание',
    'Email' => 'Электронная почта',
    'Foreign tour' => 'Забугор-вояж',
    'Go home' => 'Вернуться на главную',
    'Home page' => 'Домашняя страница',
    'Id' => 'Номер',
    'Image' => 'Картинка',
    'Is special order' => 'Спец. предложение',
    'Language' => 'Язык',
    'Login' => 'Вход',
    'MAIN NAVIGATION' => 'Навигация',
    'More details' => 'Читать далее',
    'Name' => 'Имя',
    'News' => 'Новости',
    'No results found' => 'Нет результатов для отображения',
    'Order' => 'Заказать',
    'Order no.' => 'Номер заказа',
    'Order tour' => 'Заказать тур',
    'Our manager will contact you soon.' => 'Наш менеджер скоро свяжется с вами.',
    'Parent' => 'Родитель',
    'Password' => 'Пароль',
    'Phone' => 'Телефонный номер',
    'Please fill out the following fields to login:' => 'Пожалуйста заполните следующие поля для входа:',
    'Profile' => 'Профиль',
    'Region' => 'Регион',
    'Regions' => 'Регионы',
    'Remember me' => 'Запомнить меня',
    'Search' => 'Поиск',
    'Search results' => 'Результаты поиска',
    'Search tour' => 'Искать туры',
    'Sign out' => 'Выход',
    'Special order' => 'Спецпредложение',
    'Special orders' => 'Спецпредложения',
    'Status' => 'Статус',
    'Surname' => 'Фамилия',
    'Title' => 'Название',
    'Toggle navigation' => 'Показать/скрыть навигацию',
    'Tour id' => 'Номер тура',
    'Tour subtype' => 'Подвид тура',
    'Tour subtypes' => 'Подвиды туров',
    'Tour type' => 'Вид тура',
    'Tour types' => 'Виды туров',
    'Tours' => 'Туры',
    'Type' => 'Тип',
    'Update' => 'Обновить',
    'Update city: ' => 'Обновить город: ',
    'Update content page: ' => 'Обновить страницу содержимого: ',
    'Update news item: ' => 'Обновить новость: ',
    'Update region: ' => 'Обновить страну: ',
    'Update tour type: ' => 'Обновить вид туров: ',
    'Update tour: ' => 'Обновить тур: ',
    'User Image' => 'Фото',
    'User name' => 'Имя пользователя',
    'View cities' => 'Показать города',
    'Your request is successfully sent!' => 'Ваш заказ успешно отправлен!',
    'no' => 'нет',
    'not published' => 'не опубликован',
    'published' => 'опубликован',
    'tel.' => 'тел.',
    'yes' => 'да',
];
